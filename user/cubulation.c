...
*/
void init(process_t *process) {
  process->init_window(process, 20, 20, "My Window!");
}
...
static void draw_player(window_t *window, Vector2 screen_size) {
  ...
  if (is_key_pressed(window->input_state, KEY_SPACE) && player.position.y >= screen_size.y) {
    player.velocity.y = -10;
  }

  if (is_key_down(window->input_state, KEY_A)) {
    player.velocity.x = -5;
  } else if (is_key_down(window->input_state, KEY_D)) {
    player.velocity.x = 5;
  }
  ...
  draw_rectangle(window, player.color, player.position, (Vector2){50, 50});
}
...
void update(process_t *process) {
  ...
  draw_rectangle(window, WHITE, (Vector2){0, 0}, screen_size);
